...
main(int argc, char *const *argv)
{
    ...
    //获取命令行参数选项，并将其赋值给具体的变量标志
    if (ngx_get_options(argc, argv) != NGX_OK) {
        return 1;
    }
    //ngx_show_version 标志的话，显示版本西悉尼
    if (ngx_show_version) {
    ...
    //初始化系统相关参数 如内存页大小 配置文件等
    if (ngx_process_options(&init_cycle) != NGX_OK) {
        return 1;
    }

    //操作系统的获取  页大小 系统限制 最大连接数等
    ...
    //nginx共享内存相关
    ngx_slab_sizes_init();
    //该函数通过解析环境变量 NGINX_VAR="NGINX" ，
    //将其中的 socket number 保存至 ngx_cycle.listening 数组，该数组元素类型为 ngx_listening_t
