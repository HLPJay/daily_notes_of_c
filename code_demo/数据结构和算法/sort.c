...
#include <math.h>
/**************************************
思考：实现排序，时间复杂度为n（知道最大值和最小值，做存储）
冒泡排序：交换相邻元素，最大元素换到最后。       O(n*n)
选择排序：先确定第一个最大或者最小，后面依次。 O(n*n)
插入排序：从第一个元素开始，依次往第0个元素插入。 O(n*n)
希尔排序：按增量依次进行分组，直到1个元素1组，依次进行插入排序。
归并排序：先使每个子序列有序，再使子序列段间有序。              O(n log n)
快速排序：取临界值，小的放左边，大的放右边,递归实现。          O(n log n)
堆排序：   构建大堆栈，从左到右，从下向上。                            O(n log n)
计数排序：遍历比当前数小的个数，然后构建数组存储.                 O(n+k)
桶排序：   构建max - min + 1个数组，按下标标识，进行排序。   O(n+k)
基数排序：二维数组，排序个位，十位，百位。。。                     O(n*k)
